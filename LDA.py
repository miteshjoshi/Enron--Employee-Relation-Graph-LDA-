...
#TODO: need to apply either grid search or random search to tune the parameters
...
for i in range(len(names)):
    for j in range(len(names)):
        if i != j:
            dist = 1 - spatial.distance.cosine(doc_topic[i], doc_topic[j]) #cosine computes distance so to compute dist 1 minus
            if dist > 0.6: #pruning
